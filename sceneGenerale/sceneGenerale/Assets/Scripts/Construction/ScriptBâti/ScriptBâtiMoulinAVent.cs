...
public class ScriptBâtiMoulinAVent : MonoBehaviour
{
    ...
    public GameObject prefabMoulinAVent;
    ...
    void Update() // Pour savoir quand je clique dehors oui
    {
        ...
        if (Input.GetKeyDown(KeyCode.Mouse0))
        {
            ...
            if (Physics.Raycast(ray, out hit))
            {
                ...
                if (hit.collider.CompareTag("BatiMoulinAVent"))
                {
    ...
    private void OnGUI() //affichage des menus
    {
        ...
        if (!OnCliqueDehors && OnAfficheLeMenuDuBâti)
        {
            ...
            {
                //print("oui");
                GUI.Box(new Rect(mP.x, Screen.height - mP.y, 150, 250), "MoulinAVent");   // la fonction Screen.height permet 
                ...
                if (débuterConstruction)
                {
                    ...
                    { //onGUI est une fonction qui s'appelle à chaque frame, donc il faut faire attention à ne construire le moulin qu'une seule fois
                        ...
                        BatiMoulinAVent = GameObject.Find("BatiMoulinAVent");

                        MoulinAVent = Instantiate(prefabMoulinAVent, BatiMoulinAVent.transform.position + new Vector3(0f, 2f, 0f), Quaternion.Euler(-20, 0, 0)); //Le moulin final
                        Destroy(BatiMoulinAVent);
