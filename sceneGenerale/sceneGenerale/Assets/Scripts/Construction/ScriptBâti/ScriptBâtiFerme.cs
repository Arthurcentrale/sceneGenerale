...
public class ScriptBâtiFerme : MonoBehaviour
{
    ...
    void Update() // Pour savoir quand je clique dehors oui
    {
        ...
        if (Input.GetKeyDown(KeyCode.Mouse0))
        {
            ...
            if (Physics.Raycast(ray, out hit))
            {
                ...
                if (hit.collider.CompareTag("BatiFerme"))
                {
    ...
    private void OnGUI() //affichage des menus
    {
        ...
        if (!OnCliqueDehors && OnAfficheLeMenuDuBâti)
        {
            ...
            {
                //print("oui");
                GUI.Box(new Rect(mP.x, Screen.height - mP.y, 150, 250), "Ferme");   // la fonction Screen.height permet 
