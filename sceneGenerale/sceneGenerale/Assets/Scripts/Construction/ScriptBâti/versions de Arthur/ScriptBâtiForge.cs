...
public class ScriptBâtiForge : MonoBehaviour
{
    ...
    private void OnGUI() //affichage des menus
    {
        ...
        if (!OnCliqueDehors && OnAfficheLeMenuDuBâti)
        {
            ...
            {
                //print("oui");
                GUI.Box(new Rect(mP.x, Screen.height - mP.y, 150, 250), "Forge");   // la fonction Screen.height permet 
                ...
                if (débuterConstruction)
                {
                    ...
                    { //onGUI est une fonction qui s'appelle à chaque frame, donc il faut faire attention à ne construire le moulin qu'une seule fois
                        ...
                        BatiForge = GameObject.Find("BatiForge");

                        Forge = Instantiate(prefabForge, BatiForge.transform.position + new Vector3(0f, 2f, 0f), Quaternion.Euler(-20, 0, 0)); //Le moulin final
                        Destroy(BatiForge);
